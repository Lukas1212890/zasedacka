...
function showDetail(r){selected=r;deleteArmed=false;$('#deleteBtn').textContent='Smazat';const note=r.note?'<br><br><b>Poznámka</b><br>'+esc(r.note).replace(/\\n/g,'<br>'):'';const dates=r.date_to&&r.date_to!==r.date?esc(r.date)+' – '+esc(r.date_to):esc(r.date),time=r.all_day?'Celý den':esc(r.time_from)+'–'+esc(r.time_to);$('#detail').innerHTML='<b>'+dates+'</b><br>'+time+'<br><br><b>'+esc(r.name)+'</b><br>'+esc(r.phone)+'<br>'+esc(r.email)+note;$('#editBtn').style.display=admin?'inline-block':'none';$('#deleteBtn').style.display=admin?'inline-block':'none';open('#detailModal')}
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === '/api/reservations' && request.method === 'POST') {
        ...
        await env.DB.prepare(`INSERT INTO reservations(date,date_to,all_day,time_from,time_to,name,phone,email,note) VALUES(?,?,?,?,?,?,?,?,?)`).bind(r.date,r.date_to,r.all_day?1:0,r.time_from,r.time_to,r.name,r.phone,r.email,r.note).run();
