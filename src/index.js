...
$('#newBtn').addEventListener('click',()=>showForm(iso(new Date())));$('#prev').addEventListener('click',()=>{cursor.setMonth(cursor.getMonth()-1);load()});$('#next').addEventListener('click',()=>{cursor.setMonth(cursor.getMonth()+1);load()});$('#today').addEventListener('click',()=>{cursor=new Date();cursor.setDate(1);load()});load();
</script></body></html>`;
