...
function render(){const y=cursor.getFullYear(),m=cursor.getMonth(); $('#month').textContent=cursor.toLocaleDateString('cs-CZ',{month:'long',year:'numeric'}); $('#adminBadge').classList.toggle('show',admin); $('#adminBtn').textContent=admin?'Odhlásit':'Admin'; cal.innerHTML=['Po','Út','St','Čt','Pá','So','Ne'].map(x=>'<div class="dow">'+x+'</div>').join(''); const first=new Date(y,m,1), shift=(first.getDay()+6)%7, start=new Date(y,m,1-shift); const today=iso(new Date()),now=new Date(); for(let i=0;i<42;i++){const d=new Date(start);d.setDate(start.getDate()+i);const ds=iso(d), out=d.getMonth()!==m; const items=reservations.filter(r=>r.date===ds); const div=document.createElement('div');div.className='day'+(out?' out':'')+(ds===today?' today':'');div.innerHTML='<div class="daynum">'+d.getDate()+'</div><button class="add" title="Přidat rezervaci">+</button><div class="slots"></div>'; div.querySelector('.add').addEventListener('click',()=>showForm(ds)); const slots=div.querySelector('.slots'); if(items.length===0&&!out) slots.innerHTML='<div class="empty">Volno</div>'; items.forEach(r=>{const past=new Date(r.date+'T'+r.time_to)<now;const el=document.createElement('div');el.className='slot'+(past?' past':'');el.innerHTML='<b>'+esc(r.time_from)+'–'+esc(r.time_to)+'</b><small>'+esc(r.name)+(past?'<span class="pastLabel">Proběhlo</span>':'')+'</small>';el.addEventListener('click',()=>showDetail(r));slots.appendChild(el)});cal.appendChild(div)}}
