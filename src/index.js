...
const ICON_PNG_BASE64 = '';

function iconPng() {
  const binary = atob(ICON_PNG_BASE64), bytes = new Uint8Array(binary.length);
  for (let i = 0; i < binary.length; i++) bytes[i] = binary.charCodeAt(i);
  return new Response(bytes, { headers: { 'content-type': 'image/png', 'cache-control': 'public, max-age=604800' } });
}

const SAFARI_MASK_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 180 180"><path d="M38 18h16V7h18v11h36V7h18v11h16c17 0 30 13 30 30v94c0 17-13 30-30 30H38c-17 0-30-13-30-30V48c0-17 13-30 30-30zm-12 48v76c0 7 5 12 12 12h104c7 0 12-5 12-12V66H26zm37 24h58c7 0 10 9 4 13l-37 24h35v16H64c-7 0-10-9-4-13l37-24H63V90z"/></svg>`;

function safariMask() { return new Response(SAFARI_MASK_SVG, { headers: { 'content-type': 'image/svg+xml; charset=utf-8', 'cache-control': 'public, max-age=604800' } }); }
function manifest() { return json({ name:'Zasedačka', short_name:'Zasedačka', start_url:'/', display:'standalone', background_color:'#fff5f9', theme_color:'#d94f86', icons:[{src:'/zasedacka-icon-v2.png',sizes:'180x180',type:'image/png'}] }, 200, { 'cache-control':'public, max-age=3600' }); }
...
<title>Zasedačka</title><meta name="theme-color" content="#d94f86"><link rel="icon" type="image/png" sizes="180x180" href="/zasedacka-icon-v2.png"><link rel="shortcut icon" href="/favicon.ico"><link rel="apple-touch-icon" sizes="180x180" href="/zasedacka-icon-v2.png"><link rel="mask-icon" href="/safari-pinned-tab.svg" color="#d94f86"><link rel="manifest" href="/site.webmanifest">
<style>
...
export default {
  async fetch(request, env) {
    ...
    try {
      if (['/apple-touch-icon.png','/apple-touch-icon-precomposed.png','/zasedacka-icon-v2.png','/favicon.ico'].includes(url.pathname) && request.method === 'GET') return iconPng();
      if (url.pathname === '/safari-pinned-tab.svg' && request.method === 'GET') return safariMask();
      if (url.pathname === '/site.webmanifest' && request.method === 'GET') return manifest();
